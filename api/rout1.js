// api/routes.js - The Definitive, Conflict-Free Version with Unified JSON Data
...
const logRequestToDb = async (url) => { if (!url || !process.env.DATABASE_URL) return; try { await pool.query('INSERT INTO request_logs(url) VALUES($1)', [url]); } catch (err) { console.error("DB Log Error:", err); }};
...
// --- THE NEW UNIFIED DATA ENDPOINT ---
// This single endpoint replaces the old /metadata and /formats endpoints.
// It uses a Python helper to get all data as JSON in one call, improving efficiency
// and eliminating the need for frontend parsing of raw text.
router.post('/get-data', verifyTicket, checkIfBlocked, async (req, res, next) => {
    try {
        await logRequestToDb(req.body.url);
        const { url } = req.body;
        ...
        const pythonProcess = spawn(pythonCommand, [pythonHelperPath, url]);
        ...
        pythonProcess.on('close', (code) => {
            ...
            try {
                ...
                const formats = (info.formats || [])
                    .map(f => {
                        if (!f.format_id || f.ext === 'mhtml' || (f.vcodec && f.vcodec.includes('images'))) {
                            return null;
                        }
                        
                        let filesize = f.filesize || f.filesize_approx;
                        if (!filesize && f.tbr && duration > 0) {
                            filesize = (f.tbr * 1000 / 8) * duration;
                        }

                        let filesize_str = "N/A";
                        if (filesize) {
                            const size_mib = filesize / (1024 * 1024);
                            if (size_mib >= 1000) {
                                filesize_str = `${(size_mib / 1024).toFixed(2)} GiB`;
                            } else {
                                filesize_str = `${size_mib.toFixed(1)} MiB`;
                            }
                        }
                ...
                console.error("Error parsing JSON from Python helper:", e.message);
                res.status(500).json({ error: 'Failed to parse video data from helper.' });
            }
        });
    } catch (error) {
        next(error);
...
router.get('/admin/requests', verifyAdmin, async (req, res) => { if (!process.env.DATABASE_URL) return res.json([]); try { const { rows } = await pool.query('SELECT url, timestamp FROM request_logs ORDER BY timestamp DESC LIMIT 50'); res.json(rows); } catch (err) { res.status(500).json({ error: "Failed to fetch request logs." }); } });
